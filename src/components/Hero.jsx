...
const Hero = ({ scrollYProgress }) => {
  ...
  const heroScale = useTransform(scrollYProgress, [0, 0.15], [1.2, 0.7]); // Shrinks from 1 → 0
  const heroOpacity = useTransform(scrollYProgress, [0, 0.15], [1, 0]); // Fades from 1 → 0

  // Second div (Intro section) transformations
  const introOpacity = useTransform(scrollYProgress, [0.14, 0.25], [0, 1]); // Fades in
  const introScale = useTransform(scrollYProgress, [0.14, 0.25], [3, 1.2]); // Shrinks from 2 → 1
  return (
    <>
